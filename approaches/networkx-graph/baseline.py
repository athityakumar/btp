...
def main(argv):
    ...
    OUTPUT, HELP, PATH = False, False, './../daru-dataframe/spec/fixtures/'
    ...
    for task in [1]:
        ...
        for lang in sorted(set(x.split('-train')[0] for x in os.listdir(PATH))):
            only_once = 0
            for quantity in ['medium']:
                allprules, allsrules = {}, {}
                ...
                if task == 1:
                    # if lang == 'english':
                    #     for pos in allsrules:
                    #         for (i, (key, value)) in enumerate(sorted(allsrules[pos].iteritems(), key=lambda (k,v): (v, k), reverse=True)):
                    #             if i == 0:
                    #                 print (pos, key, value)
                    #     for pos in allprules:
                    #         for (i, (key, value)) in enumerate(sorted(allprules[pos].iteritems(), key=lambda (k,v): (v, k), reverse=True)):
                    #             if i == 0:
                    #                 print (pos, key, value)
                    for l in devlines:
                ...
                print(lang + "[task " + str(task) + "/" + quantity + "]" + ": " + str(str(numcorrect/float(numguesses)))[0:7], prefbias, suffbias)
